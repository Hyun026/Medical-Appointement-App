...
class Endocrino extends StatefulWidget {
  const Endocrino({super.key});

  @override
  State<Endocrino> createState() => _EndocrinoState();
}

class _EndocrinoState extends State<Endocrino> {
 final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  Future<List<Map<String, dynamic>>> _getPediatricians() async {
    QuerySnapshot querySnapshot = await _firestore
        .collection('doctors')
        .where('field', isEqualTo: 'Endocrinologist')
        .get();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Endocrinologist Doctors'),
      ),
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const Center(child: Text('No Endocrinologist found'));
          } else {
