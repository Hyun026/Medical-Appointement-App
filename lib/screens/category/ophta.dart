...
class Ophtalmo extends StatefulWidget {
  const Ophtalmo({super.key});

  @override
  State<Ophtalmo> createState() => _OphtalmoState();
}

class _OphtalmoState extends State<Ophtalmo> {
 final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  Future<List<Map<String, dynamic>>> _getPediatricians() async {
    QuerySnapshot querySnapshot = await _firestore
        .collection('doctors')
        .where('field', isEqualTo: 'Ophthalmologist')
        .get();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Ophthalmologist'),
      ),
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const Center(child: Text('No Ophtalmologists found'));
          } else {
