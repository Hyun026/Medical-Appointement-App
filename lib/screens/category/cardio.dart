...
class Cardiolo extends StatefulWidget {
  const Cardiolo({super.key});

  @override
  State<Cardiolo> createState() => _CardioloState();
}

class _CardioloState extends State<Cardiolo> {
 final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  Future<List<Map<String, dynamic>>> _getPediatricians() async {
    QuerySnapshot querySnapshot = await _firestore
        .collection('doctors')
        .where('field', isEqualTo: 'Cardiologist')
        .get();
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Center(child: Text('No Pediatricians found'));
          } else {
