...
class MyOrthopedy extends StatefulWidget {
  const MyOrthopedy({super.key});

  @override
  State<MyOrthopedy> createState() => _MyOrthopedyState();
}

class _MyOrthopedyState extends State<MyOrthopedy> {
        ...
        actions: [
          QuickAction(
            ...
            onTap: () async {
              ...
              if (selectedImage1 != null) {
                bool success = await uploadFileOrth(selectedImage1);
                if (success) {
          ...
          QuickAction(
            ...
            onTap: () async {
              ...
              if (selectedImage != null) {
                bool success = await uploadFileOrth(selectedImage);
                if (success) {
                  await getUploadedFiles();
                }
              }
          ...
          QuickAction(
            ...
            onTap: () async {
              ...
              if (pickedFile != null) {
                bool uploadSuccess = await uploadFileOrth(pickedFile);
                if (uploadSuccess) {
  ...
  Future<void> getUploadedFiles() async {
    List<Reference>? result = await getUsersUploadedFilesOrth();
    if (result != null) {
      setState(() {
