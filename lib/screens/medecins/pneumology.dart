...
class MyPneu extends StatefulWidget {
  const MyPneu({super.key});

  @override
  State<MyPneu> createState() => _MyPneuState();
}

class _MyPneuState extends State<MyPneu> {
        ...
        actions: [
          QuickAction(
            ...
            onTap: () async {
              ...
              if (selectedImage1 != null) {
                bool success = await uploadFilePneu(selectedImage1);
                if (success) {
          ...
          QuickAction(
            ...
            onTap: () async {
              ...
              if (selectedImage != null) {
                bool success = await uploadFilePneu(selectedImage);
                if (success) {
                  await getUploadedFiles();
                }
              }
          ...
          QuickAction(
            ...
            onTap: () async {
              ...
              if (pickedFile != null) {
                bool uploadSuccess = await uploadFilePneu(pickedFile);
                if (uploadSuccess) {
  ...
  Future<void> getUploadedFiles() async {
    List<Reference>? result = await getUsersUploadedFilesPneu();
    if (result != null) {
      setState(() {
